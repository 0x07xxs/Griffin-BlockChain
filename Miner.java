...
public class Miner implements Runnable {
    ...
    public boolean doProofOfWork(Block oBlock) {
        String zeros = "";
        while (zeros.length() < oBlock.getDifficulty()) {
            zeros = zeros + "0";
        }
        ...
        while (true) {
            if (bAbortPoW == true) {
                bAbortPoW = false;
                System.out.println("Aborted mining block, because another confirmed block received.");
                return false;
            } else {
                oBlock.setNonce(nonce);
                String sComputeHash = oBlock.computeHash();
                if (sComputeHash.charAt(0) == '0') {
                    return true;
                } else {
                    return false;
                }
